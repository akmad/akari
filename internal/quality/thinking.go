...
// OMP, Grok, and OpenCode keep plaintext (~4 bytes per token, the usual English ratio; Grok
// logs summary thoughts). Codex's, Grok's, and OpenCode's exact per-turn counts are used
// directly where present, so their factors only cover a turn that somehow logged a trace but
// no token count. Cursor has no entry on purpose: its transcript never separates reasoning
// from answer text, so a Cursor turn records no thinking bytes for a divisor to act on.
