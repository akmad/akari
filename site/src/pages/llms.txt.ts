...
export const GET: APIRoute = async () => {
  ...
  out.push("");
  out.push(
    "akari collects the local session records of Claude Code, Codex, pi, OMP, Cursor, Grok, and OpenCode from every machine, parses them on one server, and shows them as a searchable history grouped by git project, with token usage and cost on every run. This is the user guide."
  );
  out.push("");
  out.push("## User guide");
  out.push("");
